...
import { createZoom } from './graph_zoom'
...
export default class extends Controller {
  ...
  #render () {
    ...
    this._zoomApi = createZoom({
      svg, vp, world,
      canvas: this.canvasTarget,
      storageGet: (k) => this.#storageGet(k),
      storageSet: (k, v) => this.#storageSet(k, v)
    })
    // Chrome (floating toolbar) is appended after zoom sets up the canvas; kept here
    // in #render rather than inside createZoom so zoom and chrome stay decoupled.
  ...
  #classPath (iri) {
    // Navigating from a graph node should keep the active language rather than
    // resetting to the portal default. Carry it forward from the current page URL.
    let url = `/ontologies/${encodeURIComponent(this.ontologyValue)}?p=classes&conceptid=${encodeURIComponent(iri)}`
